...
class Monster:
    def __init__(self, debugMode, logging):
        ...
        self.logging.info("Creating Monster object...")
        self.engineName = "Monster"

    # The start of the job search process.
    # sample URL
    # https://www.monster.com/jobs/search/?q=Entry-IT&where=Waltham__2C-MA&sort=rv.dt.di&page=2
    def jobSearch(self, chrome, lists, resultsList, positionLevel, keyword, location):
        ...
        searchPageURL = "https://www.monster.com/jobs/search/?"
        ...
        try:
            ...
            locationText = locationText.replace(",", "__2C")
            searchPageURL = searchPageURL + ("q={0}-{1}&where={2}".format(positionText, keywordText, locationText))
        ...
        try:
            ...
            searchPageURL = searchPageURL + "&sort=rv.dt.di&page=1"
        ...
        for pageIndex in range(1, 4):
            try:
                ...
                searchPageURL = (searchPageURL[:searchPageURL.index("&page=")] + "&page={0}".format(pageIndex))
                chrome.get(searchPageURL)
                searchPageURL = chrome.current_url
                self.logging.debug("Page reached!")
            except Exception as error:
                self.logging.info("End of job pages.")
                self.logging.warning("Could not navigate to page {0} of jobs.".format(pageIndex))
                self.logging.error(error)
                break

            # Monster does max 26 postings per page. It starts on index 2 for some reason though? Idk either.
            # Get a list of the hrefs that we wanna parse through.
            for jobIndex in range(2, 28):
    ...
    def previewScreen(self, chrome, lists, jobIndex):
        ...
        try:
            url = chrome.find_element_by_xpath("//*[@id=\"resultsWrapper\"]/div[{0}]/article/div[1]/div[1]/div[1]/h2/a".format(jobIndex)).get_attribute("href")
        except Exception as error:
        ...
        try:
            title = chrome.find_element_by_xpath("//*[@id=\"resultsWrapper\"]/div[{0}]/article/div[1]/div[1]/div[1]/h2/a".format(jobIndex)).text
        except Exception as error:
            self.logging.warning("Issue screening for Job Title.")
            self.logging.debug(error)

        try:
            location = chrome.find_element_by_xpath("//*[@id=\"resultsWrapper\"]/div[{0}]/article/div[1]/div[1]/div[3]/p/a".format(jobIndex)).text
        except Exception as error:
            self.logging.warning("Issue screening for Job Location.")
            self.logging.debug(error)

        try:
            timePostedAgo = chrome.find_element_by_xpath("//*[@id=\"resultsWrapper\"]/div[{0}]/article/div[1]/div[3]/p/time".format(jobIndex)).text
        except Exception as error:
    ...
    def pageParse(self, chrome, lists):
        ...
        try:
            # Job title
            try:
                element = chrome.find_element_by_xpath("//div[@id='JobViewHeader']/header[@class='card-content job-caption']/div[@class='heading']/h1[@class='title']")
                try:
                    title = element[:element.text.index(" at ")]
                    title = element[:element.text.index(" from ")]
                except:
                    title = element.text
            ...
            # Time Posted
            try:
                element = chrome.find_element_by_xpath("//div[@class='mux-job-summary']/section[@class='summary-section '][4]/dl[@class='header']/dd[@class='value']")
            ...
            # Job Location
            try:
                element = chrome.find_element_by_xpath("//div[@id='JobViewHeader']/header[@class='card-content job-caption']/div[@class='heading']/h2[@class='subtitle']")
            ...
            # Job Description
            try:
                element = chrome.find_element_by_id("JobBody")
