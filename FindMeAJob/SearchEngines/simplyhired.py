...
class SimplyHired:
    def __init__(self, debugMode, logging):
        ...
        self.engineName = "SimplyHired"

    # sample URL
    # https://www.simplyhired.com/search?q=entry+it&l=waltham%2C+ma&fdb=7
    def jobSearch(self, chrome, lists, resultsList, positionLevel, keyword, location):
        ...
        searchPageURL = "https://www.simplyhired.com/"
        ...
        try:
            ...
            searchPageURL = searchPageURL + ("search?q={0}+{1}&l={2}".format(positionText, keywordText, locationText))
        ...
        try:
            ...
            searchPageURL = searchPageURL + "&fdb=7&pn=1"
        ...
        for pageIndex in range(1, 4):
            try:
                ...
                searchPageURL = (searchPageURL[:searchPageURL.index("&pn=")] + "&pn={0}".format(pageIndex))
                chrome.get(searchPageURL)
            ...
            # Job listings are difficult to find iteratively through xpath or id so we need to tab through the page. Tab 17 times to get to the first listing. We subtract one because we will be tabbing in each iteration of the next loop.
            try:
                self.logging.debug("Attempting to tab to first job...")
                tabBig = ActionChains(chrome)
                tabBig.send_keys(Keys.TAB * 16)
                tabBig.perform()
            ...
            try:
                ...
                for x in range(0, 250):
                    ...
                    try:
                        # Check to see if we are at the end. If we hit the Privacy then we are at the end
                        if element.text == "Privacy":
                            self.logging.debug("End of results page!")
                            break
                        # Otherwise, add this element to the jobPageList for later parsing
                        elif element.get_attribute("href").startswith("https://www.simplyhired.com/job/") and len(element.text) > 5:
            ...
            # CareerBuilder does max 26 postings per page
    ...
    def pageParse(self, chrome, lists):
        ...
        try:
            # Job title
            try:
                element = chrome.find_element_by_xpath("//div[@class='viewjob-header-bar']/div[@class='viewjob-header']/h1")
                title = element.text
            ...
            # Time Posted
            try:
                element = chrome.find_element_by_xpath("//div[@class='viewjob-header-bar']/div[@class='viewjob-header']/h2/span[@class='after-company']/span[@class='age']")
            ...
            # Job Location
            try:
                element = chrome.find_element_by_xpath("//div[@class='viewjob-header-bar']/div[@class='viewjob-header']/h2/span[@class='after-company']/span[@class='location']")
            ...
            # Job Description
            try:
                element = chrome.find_element_by_xpath("//div[@class='viewjob-paper']/div[@class='viewjob-description']")
